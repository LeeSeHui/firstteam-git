...
type VoteItem = {
  id: number;
  img: string;
  text: string; // contains <br /> for HTML line breaks
};

type UseNicknameReturn = { nickname: string };

const Home: React.FC = () => {
  const navigate = useNavigate();
  const { nickname } = (useNickname() as UseNicknameReturn) || { nickname: '' }; 
  ...
  const handleAddComment = () => {
    ...
    const newObj: CommentItem = { username: { nickname }, message: newComment, createdAt: new Date().toISOString() };
  ...
  return (
    <div className="homeContainer">
      <div className="main-logo">
        ...
        <p className="img2 alarm" onClick={AlarmClick}>
      ...
      <div className="liveSection">
        <p>LIVE NOW!</p>
        <Swiper spaceBetween={16} slidesPerView="auto" grabCursor modules={[]}>
      ...
      <div className="videoSection">
        ...
        <Swiper spaceBetween={65} slidesPerView="auto" grabCursor modules={[]}>
          {[thumb1, thumb2, thumb3].map((thumb, i) => {
            const slideStyle: CSSProperties = { width: '190px', height: '200px' };
            const hrefs = [
              'https://www.youtube.com/watch?v=yFTAYXDTjqI',
              'https://www.youtube.com/watch?v=lmJPeFW75qQ',
            ...
            return (
              <SwiperSlide key={i} style={slideStyle}>
                <a href={hrefs[i]} target="_blank" rel="noopener noreferrer" className="videoCard">
                  ...
                  <p dangerouslySetInnerHTML={{ __html: captions[i] }} />
                </a>
              </SwiperSlide>
            );
          })}
        ...
        >
          {voteData.map((vote, idx) => (
            <SwiperSlide key={vote.id} style={{ width: '290px' }}>
              <div
                className={
                  idx <= 2 ? 'shift-wrapper shift-right' : idx >= 3 && idx <= 5 ? 'shift-wrapper shift-left' : ''
                }
              >
